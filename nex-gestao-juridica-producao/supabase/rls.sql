...
returns boolean language sql security definer set search_path = public stable as $$
  select coalesce(public.current_profile_role() in ('admin','socio','financeiro'), false)
$$;

create or replace function public.can_access_hr()
returns boolean language sql security definer set search_path = public stable as $$
  select coalesce(public.current_profile_role() in ('admin','socio','rh'), false)
$$;

create or replace function public.can_access_process(process_responsible uuid default null)
returns boolean language sql security definer set search_path = public stable as $$
  select coalesce(public.current_profile_role() in ('admin','socio','controladoria') or process_responsible = public.current_profile_id(), false)
$$;

create or replace function public.can_access_client(client_responsible uuid default null)
returns boolean language sql security definer set search_path = public stable as $$
  select coalesce(public.current_profile_role() in ('admin','socio','atendimento','controladoria','financeiro') or client_responsible = public.current_profile_id(), false)
$$;
...
create policy org_select_employees on public.employees for select using (organization_id = public.current_profile_org() and (public.can_access_hr() or profile_id = public.current_profile_id() or public.current_profile_role() in ('admin','socio')));
...
create policy org_modify_clients on public.clients for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','atendimento','advogado','controladoria')) with check (organization_id = public.current_profile_org());

-- LEADS
drop policy if exists org_select_leads on public.leads;
create policy org_select_leads on public.leads for select using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','atendimento','advogado'));
drop policy if exists org_modify_leads on public.leads;
create policy org_modify_leads on public.leads for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','atendimento','advogado')) with check (organization_id = public.current_profile_org());

-- PROCESSOS
drop policy if exists org_select_processes on public.processes;
create policy org_select_processes on public.processes for select using (organization_id = public.current_profile_org() and (public.current_profile_role() in ('admin','socio','advogado','controladoria','financeiro') or responsible_id = public.current_profile_id()));
drop policy if exists org_modify_processes on public.processes;
create policy org_modify_processes on public.processes for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','controladoria')) with check (organization_id = public.current_profile_org());

-- PRAZOS
drop policy if exists org_select_deadlines on public.deadlines;
create policy org_select_deadlines on public.deadlines for select using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','controladoria'));
drop policy if exists org_modify_deadlines on public.deadlines;
create policy org_modify_deadlines on public.deadlines for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','controladoria')) with check (organization_id = public.current_profile_org());

-- TAREFAS
drop policy if exists org_select_tasks on public.tasks;
create policy org_select_tasks on public.tasks for select using (organization_id = public.current_profile_org() and (responsible_id = public.current_profile_id() or public.current_profile_role() in ('admin','socio','advogado','controladoria','atendimento','financeiro','rh')));
drop policy if exists org_modify_tasks on public.tasks;
create policy org_modify_tasks on public.tasks for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','controladoria','atendimento','financeiro','rh')) with check (organization_id = public.current_profile_org());
...
create policy org_select_pricing on public.pricing_proposals for select using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','financeiro','atendimento'));
drop policy if exists org_modify_pricing on public.pricing_proposals;
create policy org_modify_pricing on public.pricing_proposals for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','financeiro')) with check (organization_id = public.current_profile_org());
...
create policy org_modify_documents on public.documents for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','controladoria','cliente')) with check (organization_id = public.current_profile_org());
...
create policy org_modify_automations on public.automation_rules for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','controladoria')) with check (organization_id = public.current_profile_org());
...
create policy org_modify_pricing_templates on public.pricing_templates for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','financeiro','advogado')) with check (organization_id = public.current_profile_org());
...
create policy org_select_hearings on public.hearings for select using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','controladoria','cliente'));
drop policy if exists org_modify_hearings on public.hearings;
create policy org_modify_hearings on public.hearings for all using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','advogado','controladoria')) with check (organization_id = public.current_profile_org());
...
create policy org_select_report_exports on public.report_exports for select using (organization_id = public.current_profile_org() and public.current_profile_role() in ('admin','socio','financeiro','rh','controladoria','advogado'));
...
create policy org_select_processes on public.processes for select using (
  organization_id = public.current_profile_org()
  and (
    public.current_profile_role() in ('admin','socio','advogado','controladoria','financeiro')
...
create policy org_select_deadlines on public.deadlines for select using (
  organization_id = public.current_profile_org()
  and (
    public.current_profile_role() in ('admin','socio','advogado','controladoria')
    or (public.current_profile_role() = 'cliente' and client_id = public.current_profile_client_id())
  )
);
...
create policy org_select_tasks on public.tasks for select using (
  ...
  and (
    ...
    or public.current_profile_role() in ('admin','socio','advogado','controladoria','atendimento','financeiro','rh')
    or (public.current_profile_role() = 'cliente' and client_id = public.current_profile_client_id())
  )
);
...
create policy org_select_pricing on public.pricing_proposals for select using (
  organization_id = public.current_profile_org()
  and (
    public.current_profile_role() in ('admin','socio','advogado','financeiro','atendimento')
    or (public.current_profile_role() = 'cliente' and client_id = public.current_profile_client_id())
  )
);
...
create policy org_modify_pricing on public.pricing_proposals for all using (
  organization_id = public.current_profile_org()
  and (
    public.current_profile_role() in ('admin','socio','advogado','financeiro')
    or (public.current_profile_role() = 'cliente' and client_id = public.current_profile_client_id())
  )
