...
function mapFromSupabase(state: AppState, entity: EntityName, rows: any[]): AppState {
  ...
  if (entity === "tasks") next.tasks = rows.map((r) => ({ id: r.id, title: r.title, description: r.description ?? "", processId: r.process_id ?? "", clientId: r.client_id ?? undefined, client: clientName(state, r.client_id) || r.client_name || "", responsible: r.responsible_id ?? "", sector: r.sector ?? "", priority: r.priority ?? "Média", status: r.status ?? "Pendente", due: String(r.due_at ?? "").slice(0, 10), estimatedHours: Number(r.estimated_hours ?? 0), spentHours: Number(r.spent_hours ?? 0), checklist: r.checklist ?? [], comments: r.comments ?? [], archivedAt: r.archived_at ?? undefined }));
...
export async function loadNormalizedState(): Promise<AppState> {
  ...
  let next = emptyState();
  const order: EntityName[] = ["employees", "clients", "leads", "processes", "deadlines", "tasks", "finances", "documents", "messages", "automations", "automationRuns", "timeRecords", "pricings", "payrolls", "integrations", "hearings", "clientConsents", "paymentReceipts", "reportExports", "auditLogs"];
  for (const entity of order) {
    const table = tableNames[entity];
    if (!table) continue;
    let query = supabase.from(table).select("*").limit(700);
    const orgId = getCurrentOrganizationId();
    if (orgId && !["automation_runs"].includes(table)) query = query.eq("organization_id", orgId);
...
function toRemotePayload(entity: EntityName, item: any, state: AppState) {
  const base = { id: item.id, organization_id: getCurrentOrganizationId(), archived_at: item.archivedAt ?? null } as Record<string, unknown>;
  ...
  if (entity === "messages") return { ...base, client_id: clientIdForPayload(state, item), process_id: item.processId || null, channel: item.channel, subject: item.subject, body: item.body ?? item.subject, status: String(item.status).toLowerCase(), scheduled_at: item.date ? new Date(`${item.date}T12:00:00`).toISOString() : null };
  ...
  if (entity === "auditLogs") return { id: item.id, organization_id: getCurrentOrganizationId(), module: item.module, action: item.action, entity_id: item.entityId, after_data: { detail: item.detail, user: item.user }, created_at: item.date ? new Date(`${item.date}T12:00:00`).toISOString() : new Date().toISOString() };
