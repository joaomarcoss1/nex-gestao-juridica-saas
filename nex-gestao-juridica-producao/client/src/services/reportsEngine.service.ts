...
export function financialSummary(state: AppState) {
  const receitas = state.finances.filter((f) => f.type === "Receita").reduce((sum, f) => sum + f.amount, 0);
  const despesas = state.finances.filter((f) => f.type === "Despesa").reduce((sum, f) => sum + f.amount, 0);
  const recebido = state.finances.filter((f) => f.status === "Pago" && f.type === "Receita").reduce((sum, f) => sum + (f.paidAmount ?? f.amount), 0);
  const vencido = state.finances.filter((f) => f.status !== "Pago" && f.dueDate < todayIso()).reduce((sum, f) => sum + f.amount, 0);
...
export function processProfitability(state: AppState, processId: string) {
  const rows = state.finances.filter((f) => f.processId === processId);
...
export function buildClientReport(state: AppState, clientName: string) {
  ...
  const finances = state.finances.filter((f) => f.client === clientName);
