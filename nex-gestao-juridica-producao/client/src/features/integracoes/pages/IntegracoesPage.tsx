...
export function IntegracoesPage({ state, commit, remove, notify }: FeaturePageProps) {
  ...
  async function test(item: Integration) {
    ...
    try {
      const response = await fetch("/api/integrations/test", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ provider: item.provider, integrationId: item.id }) });
