...
function addBusinessDays(start: string, days: number, business = true) {
  const date = new Date(`${start || todayIso()}T12:00:00`);
  let remaining = Math.max(0, days);
  while (remaining > 0) {
    date.setDate(date.getDate() + 1);
    const day = date.getDay();
    if (!business || (day !== 0 && day !== 6)) remaining -= 1;
  }
...
function blankDeadline(client: string, processId: string): Deadline {
  ...
  return { id: uid("prazo"), processId, client, responsible: "00000000-0000-4000-8000-0000000000e2", type: "Prazo processual", publicationDate: start, awarenessDate: start, startDate: start, days: 5, countType: "Dias úteis", dueDate: addBusinessDays(start, 5, true), fatal: true, priority: "Alta", status: "Pendente", notes: "Validar prazo conforme tribunal competente." };
}
...
export function PrazosPage({ state, commit, remove, notify }: FeaturePageProps) {
  ...
  async function save(deadline: Deadline) {
    const computed = { ...deadline, dueDate: deadline.dueDate || addBusinessDays(deadline.startDate, deadline.days, deadline.countType === "Dias úteis") };
  ...
  async function complete(deadline: Deadline) {
    ...
    const task: Task = { id: uid("task"), title: `Baixa de prazo: ${deadline.type}`, processId: deadline.processId, client: deadline.client, responsible: deadline.responsible, sector: "Controladoria", priority: "Média", status: "Concluída", due: todayIso(), estimatedHours: 0.25, spentHours: 0.25 };
