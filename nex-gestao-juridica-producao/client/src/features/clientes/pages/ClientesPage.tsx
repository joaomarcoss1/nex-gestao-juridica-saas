    ...
    <Panel>
      ...
      <div className="data-grid">
        {filteredClients.map((client) => <article className="data-card floating-card" key={client.id}>
          ...
          <ActionBar><Button variant="ghost" onClick={() => { history.pushState({}, "", `/clientes/${client.id}`); dispatchEvent(new PopStateEvent("popstate")); }}>Ver ficha</Button><Button variant="ghost" onClick={() => setEditingClient(client)}><Edit3 size={15}/> Editar</Button><Button variant="danger" onClick={() => deleteClient(client)}><Trash2 size={15}/> Excluir</Button></ActionBar>
        </article>)}
      </div>
    </Panel>
