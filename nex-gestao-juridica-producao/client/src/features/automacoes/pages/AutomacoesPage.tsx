  ...
  async function executeAll() {
    ...
    for (const rule of state.automations.filter((item) => item.status === "Ativa")) {
      if (rule.trigger.includes("Processo sem movimentação")) {
        ...
        const overdue = state.finances.filter((f) => f.status !== "Pago" && f.dueDate < todayIso());
        for (const entry of overdue) {
          const task: Task = { id: uid("task"), title: `Financeiro: cobrar ${entry.client} - ${money(entry.amount)}`, processId: entry.processId ?? "", client: entry.client, responsible: "e4", sector: "Financeiro", priority: "Alta", status: "Pendente", due: todayIso(), estimatedHours: 0.5, spentHours: 0 };
          await commit("tasks", task);
          await commit("finances", { ...entry, status: "Atrasado" } as FinanceEntry, "update");
          total += 1;
        }
