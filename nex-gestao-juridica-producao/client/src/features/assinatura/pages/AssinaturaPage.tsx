...
export function AssinaturaPage({ state, commit, notify }: FeaturePageProps) {
  ...
  async function enableManualTrial() {
    ...
    const reason = window.prompt("Motivo ou observação do teste gratuito:", "Teste comercial liberado pelo Admin Master Global") || "Teste comercial liberado pelo Admin Master Global";
  ...
  async function disableManualTrial() {
    if (!isMaster || !organization) return;
    const reason = window.prompt("Informe o motivo para desativar o teste gratuito:", "Teste gratuito encerrado pelo Admin Master Global") || "Teste gratuito encerrado pelo Admin Master Global";
    await updateOrganization({
      ...organization,
  ...
  async function grantLifetimeExemption() {
    if (!isMaster || !organization) return;
    const reason = window.prompt("Motivo da isenção permanente de cobrança:", "Isenção permanente concedida pelo Admin Master Global") || "Isenção permanente concedida pelo Admin Master Global";
    await updateOrganization({
      ...organization,
  ...
  async function removeLifetimeExemption() {
    if (!isMaster || !organization) return;
    const reason = window.prompt("Motivo para remover a isenção permanente:", "Isenção removida pelo Admin Master Global") || "Isenção removida pelo Admin Master Global";
    await updateOrganization({
      ...organization,
      billingMode: "stripe",
      billingExemptForever: false,
  ...
  async function checkout(plan: "starter" | "pro" | "enterprise") {
    ...
    await startStripeCheckout({ organizationId: organization.id, plan, email: organization.email, organizationName: organization.tradeName || organization.name });
  }
