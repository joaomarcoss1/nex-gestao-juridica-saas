...
function blankProcess(client: string): Process {
  return { id: uid("process"), cnj: "", type: "Judicial", client, opposite: "", activePole: client, passivePole: "", subject: "", area: "Civil", court: "", courtDivision: "", district: "", state: "MA", class: "Serviço jurídico", phase: "Atendimento inicial", status: "Em análise", risk: "Médio", successChance: 60, value: 0, fees: 0, costs: 0, responsible: "e1", nextDeadline: new Date(Date.now() + 86400000 * 7).toISOString().slice(0, 10), lastMoveDays: 0, progress: 10, checklist: ["Procuração", "Contrato de honorários", "Documentos iniciais"] };
}
...
export function ProcessosPage({ state, commit, remove, notify, setPage }: FeaturePageProps) {
  ...
  const processFields: FieldConfig<Process>[] = [
    ...
    { key: "administrativeBody", label: "Órgão administrativo" },
    { key: "protocolNumber", label: "Número do protocolo" },
    { key: "procedureType", label: "Tipo do procedimento" },
    ...
    { key: "requirements", label: "Exigências", kind: "textarea" },
    { key: "decisions", label: "Decisões", kind: "textarea" },
    { key: "appeals", label: "Recursos", kind: "textarea" },
    { key: "agreements", label: "Acordos", kind: "textarea" },
    { key: "contracts", label: "Contratos", kind: "textarea" },
