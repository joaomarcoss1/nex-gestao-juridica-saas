...
export function ProcessoDetalhePage({ state, processId, commit, executeAtomic, notify, setPage }: FeaturePageProps & { processId: string }) {
  ...
  const movements = useMemo(() => state.processMovements.filter((m) => m.processId === targetProcess.id).sort((a,b) => b.occurredAt.localeCompare(a.occurredAt)), [state.processMovements, targetProcess.id]);
  ...
  async function closeProcess() {
    if (!await requestConfirmation("Encerrar processo", "O sistema validará tarefas, prazos e workflows pendentes. Deseja continuar?")) return;
    await executeAtomic({ type: "closeProcess", processId: targetProcess.id, reason: "Encerramento confirmado pelo responsável na ficha do processo.", idempotencyKey: `process:close:${targetProcess.id}:${targetProcess.version ?? 0}`, expectedVersion: targetProcess.version });
    notify({ tone: "success", title: "Processo encerrado", message: "O encerramento foi auditado e só ocorreu após a validação de pendências." });
  }
  ...
  return <div className="page-grid">
    ...
    <Panel><PanelTitle title="Movimentações persistentes" subtitle="Histórico separado do formulário e idempotente para integrações."/><ul className="timeline-list">{movements.map(m => <li key={m.id}><b>{m.title}</b><span>{new Date(m.occurredAt).toLocaleString("pt-BR")} · {m.source}</span><small>{m.description}</small></li>)}</ul></Panel>
