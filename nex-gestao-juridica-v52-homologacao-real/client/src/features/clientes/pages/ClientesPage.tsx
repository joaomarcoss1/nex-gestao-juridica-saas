...
export function ClientesPage({ state, commit, remove, executeAtomic, notify, setPage }: FeaturePageProps) {
  ...
  async function deleteClient(client: Client) {
    if (!await requestConfirmation("Arquivar cliente", `O cliente ${client.name} será arquivado, preservando vínculos e auditoria. Deseja continuar?`)) return;
    await remove("clients", client.id);
    notify({ tone: "info", title: "Cliente removido", message: "O registro foi excluído do cadastro." });
  }

  async function deleteLead(lead: Lead) {
    if (!await requestConfirmation("Arquivar lead", `O lead ${lead.name} será arquivado. Deseja continuar?`)) return;
  ...
  async function convertLead(lead: Lead) {
    ...
    if (matches.length > 1) {
      notify({ tone: "error", title: "Correspondência ambígua", message: "Revise os identificadores do lead antes de converter." });
      return;
    }
    let reuseClientId: string | undefined;
    let duplicateOverrideReason: string | undefined;
    if (matches.length === 1) {
      if (!await requestConfirmation("Reutilizar cliente", `Deseja vincular o lead ao cadastro ${matches[0].name}?`)) return;
      reuseClientId = matches[0].id;
      duplicateOverrideReason = "Reutilização confirmada na central de clientes.";
    }
    await executeAtomic({ type: "convertLead", leadId: lead.id, reuseClientId, requireConsent: true, duplicateOverrideReason, idempotencyKey: lead.conversionIdempotencyKey || `crm:${lead.id}` });
    notify({ tone: "success", title: "Lead convertido", message: "Cliente, processo e workflow foram criados ou reutilizados de forma atômica." });
    setPage("processos");
  }
