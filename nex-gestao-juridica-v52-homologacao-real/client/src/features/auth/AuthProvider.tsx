...
import { clearStoredPortalPayload } from "@/services/portal.service";
...
type AuthContextValue = {
  ...
  signInClientPortal: (fullName: string, cpf: string, email: string, registrationCode?: string) => Promise<void>;
...
function demoProfileForRole(role: DemoRole, email?: string): AuthProfile {
  ...
  if (role === "cliente") return { ...base, id: "00000000-0000-4000-8000-0000000000c1", name: "Cliente", role: "cliente", sector: "Portal do Cliente", clientId: "00000000-0000-4000-8000-0000000000c1" };
  if (role === "funcionario") return { ...base, id: "00000000-0000-4000-8000-0000000000e5", name: "Equipe do escritório", role: "funcionario", sector: "Operacional", organizationRegistrationCode: "3272026", organizationName: "Escritório" };
...
async function ensureProfile(session: Session | null, registrationCode?: string): Promise<AuthProfile | null> {
  ...
  if (!data) throw new Error("Usuário autenticado, mas sem perfil vinculado. Solicite ao administrador do escritório a vinculação do seu acesso.");
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInClientPortal = useCallback(async (fullName: string, cpf: string, email: string, registrationCode?: string) => {
    const normalized = fullName.trim().replace(/\s+/g, " ");
    const normalizedCpf = cpf.replace(/\D/g, "");
    const normalizedEmail = email.trim().toLowerCase();
    if (normalized.length < 8) throw new Error("Informe o nome completo cadastrado no escritório.");
    if (normalizedCpf.length !== 11) throw new Error("Informe o CPF completo do cliente.");
    if (!normalizedEmail.includes("@")) throw new Error("Informe o e-mail cadastrado no escritório.");
    ...
    const response = await fetch("/api/portal/request-access", {
      ...
      body: JSON.stringify({
        ...
        registrationCode: registrationCode?.replace(/\D/g, "") || undefined,
        redirectTo: location.origin,
      }),
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok || body?.ok === false) throw new Error(body?.message || "Não foi possível solicitar o acesso seguro.");
    setSession(null);
    setProfile(null);
    setRuntimeAuthProfile(null);
    setError(undefined);
