...
import { requestConfirmation, requestTextInput } from "@/services/dialog.service";
...
export function DocumentosPage({ state, commit, remove, notify }: FeaturePageProps) {
  ...
  async function handleFile(file?: File) {
    ...
    let hash = "";
    try {
      const clientId = state.clients.find((item) => item.name === client)?.id;
      const uploaded = await uploadPrivateDocument(file, id, clientId, processId);
      storagePath = uploaded.path; hash = uploaded.hash;
    } catch (error) {
      if (databaseMode === "production") { notify({ tone: "error", title: "Upload não concluído", message: error instanceof Error ? error.message : "O arquivo não foi salvo no Storage." }); return; }
      console.warn(error);
    }
  ...
  async function rejectDoc(doc: LegalDoc) {
    const reason = await requestTextInput("Solicitar correção", "Informe o motivo da recusa ou correção necessária:", "Correção solicitada");
    if (!reason) return;
  ...
  async function deleteDoc(doc: LegalDoc) {
    if (!await requestConfirmation("Arquivar documento", `O documento ${doc.name} será arquivado. O arquivo no Storage só será removido por ação administrativa específica.`)) return;
