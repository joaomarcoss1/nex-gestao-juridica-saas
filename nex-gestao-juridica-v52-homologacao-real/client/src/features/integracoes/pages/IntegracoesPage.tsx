...
export function IntegracoesPage({ state, commit, remove, notify }: FeaturePageProps) {
  ...
  async function test(item: Integration) {
    let next: Integration = { ...item, lastSync: todayIso(), status: item.status === "Desconectado" ? "Preparado" : item.status };
    try {
      const response = await authenticatedFetch("/api/integrations/test", { method: "POST", body: JSON.stringify({ provider: item.provider, integrationId: item.id }) });
      const body = await readApiJson<{ ok?: boolean; message?: string }>(response).catch(() => null);
      if (body?.ok) next = { ...next, status: "Conectado", description: `${item.description} Teste serverless: credenciais mínimas presentes.` };
      else next = { ...next, status: "Preparado", description: `${item.description} Teste serverless: ${body?.message ?? "faltam credenciais ou backend"}` };
    } catch {
      next = { ...next, status: "Preparado" };
    }
    await commit("integrations", next, "update");
    notify({ tone: "info", title: "Teste de conexão registrado", message: "O teste verifica variáveis de ambiente no backend sem expor chaves no frontend." });
  }
