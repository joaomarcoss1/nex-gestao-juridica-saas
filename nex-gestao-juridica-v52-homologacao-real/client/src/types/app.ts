...
export type Lead = Archiveable & {
  ...
  convertedAt?: string;
  convertedBy?: string;
  conversionClientId?: string;
  conversionProcessId?: string;
  conversionStatus?: "pending" | "completed" | "failed";
  conversionIdempotencyKey?: string;
  document?: string;
  consentAccepted?: boolean;
  checklist?: Array<{ id: string; label: string; done: boolean; completedAt?: string }>;
  version?: number;
};
...
export type FinanceEntry = Archiveable & {
  ...
  paidDate?: string;
  paidAmount?: number;
  status: "Pendente" | "Pago" | "Atrasado" | "Cancelado" | "Parcial" | "Arquivado" | "Renegociado" | "Estornado" | "Baixado como perda";
  ...
  notes?: string;
  sourceType?: "manual" | "contract_installment" | "cost_entry" | "payroll" | "proposal" | "refund";
  sourceId?: string;
...
export type FeeContract = Archiveable & {
  ...
  signedAt?: string;
  firstDueDate?: string;
  dueDatePolicy?: "keep" | "previous_business_day" | "next_business_day";
  financialGeneratedAt?: string;
  version?: number;
};

export type CostEntry = Archiveable & { id: string; clientId?: string; processId?: string; financialEntryId?: string; version?: number; category: "Custas" | "Guias" | "Diligência" | "Correspondente" | "Reembolso" | "Despesa interna"; description: string; amount: number; dueDate: string; status: "Pendente" | "Pago" | "Reembolsado" | "Cancelado"; responsibleId?: string; };
...
export type LeadSource = Archiveable & {
  id: string;
  organizationId?: string;
  provider: string;
  sourceName: string;
  status: "prepared" | "configured" | "connected" | "error" | "inactive";
  defaultArea: string;
...
export type ScheduledEventSource = "manual" | "meeting" | "hearing" | "task" | "deadline" | "financial_entry" | "process" | "google_calendar";

export type ScheduledEvent = Archiveable & {
  id: string;
  organizationId?: string;
  eventType: "meeting" | "hearing" | "task" | "deadline" | "payment" | "manual" | string;
  sourceType?: ScheduledEventSource;
  sourceId?: string;
  title: string;
  clientId?: string;
  processId?: string;
  ...
  notes?: string;
  clientVisible?: boolean;
  visibility?: "internal" | "team" | "client" | "public";
  externalEventId?: string;
  recurrenceRule?: string;
  recurrenceSeriesId?: string;
  recurrenceExceptionDate?: string;
  timezone?: string;
  syncStatus?: "local" | "pending" | "synced" | "error" | "conflict" | "removed";
  lastSyncAt?: string;
  syncError?: string;
...
export type WorkflowRun = Archiveable & {
  id: string; organizationId?: string; workflowTemplateId: string; processId?: string; clientId?: string; leadId?: string;
  status: "pending" | "active" | "completed" | "cancelled" | "failed"; currentStepOrder: number; startedAt: string;
  completedAt?: string; cancelledAt?: string; startedBy?: string; idempotencyKey: string; metadata?: Record<string, unknown>; version?: number;
};
export type WorkflowRunStep = Archiveable & {
  id: string; organizationId?: string; workflowRunId: string; workflowStepId: string; taskId?: string; stepOrder: number;
  status: "pending" | "available" | "in_progress" | "blocked" | "awaiting_review" | "completed" | "skipped" | "cancelled" | "failed";
  dependsOnStepId?: string; executionGroup?: string; conditionType?: string; conditionPayload?: Record<string, unknown>;
  requiredApproverRole?: string; requiredDocumentType?: string; slaDueAt?: string; slaPausedAt?: string; slaPausedMinutes?: number;
  startedAt?: string; completedAt?: string; skippedAt?: string; blockedReason?: string; result?: Record<string, unknown>; version?: number;
};
export type ProcessMovement = Archiveable & {
  id: string; organizationId?: string; processId: string; movementType: string; title: string; description?: string; occurredAt: string;
  source: "manual" | "simulation" | "court_integration" | "workflow"; externalId?: string; visibility: "internal" | "client"; createdBy?: string;
  legacyProvider?: string; legacyRawPayload?: Record<string, unknown>;
};
export type ProcessPhaseHistory = Archiveable & {
  id: string; organizationId?: string; processId: string; previousPhase: string; newPhase: string; changedBy?: string; changedAt: string;
  reason: string; workflowRunId?: string;
};
export type ContractInstallment = Archiveable & {
  id: string; organizationId?: string; contractId: string; installmentNumber: number; amountCents: number; dueDate: string;
  status: "pending" | "partially_paid" | "paid" | "overdue" | "cancelled" | "renegotiated"; financialEntryId?: string;
};
export type FinancialPayment = Archiveable & {
  id: string; organizationId?: string; financialEntryId: string; amountCents: number; paymentDate: string; paymentMethod: string;
  reference?: string; receiptId?: string; idempotencyKey: string; createdBy?: string; cancelledAt?: string; cancellationReason?: string;
};
export type AtomicOperation =
  | { type: "convertLead"; leadId: string; idempotencyKey: string; reuseClientId?: string; requireConsent?: boolean; duplicateOverrideReason?: string }
  | { type: "createProcess"; process: Process; startWorkflowTemplateId?: string; idempotencyKey: string }
  | { type: "startWorkflow"; processId: string; workflowTemplateId: string; idempotencyKey: string; restartReason?: string }
  | { type: "completeWorkflowStep"; runStepId: string; workedMinutes: number; billableMinutes?: number; idempotencyKey: string }
  | { type: "changeProcessPhase"; processId: string; newPhase: string; reason: string; idempotencyKey: string; expectedVersion?: number }
  | { type: "closeProcess"; processId: string; reason: string; force?: boolean; idempotencyKey: string; expectedVersion?: number }
  | { type: "simulateJudicialSync"; processId: string; idempotencyKey: string }
  | { type: "saveFeeContract"; contract: FeeContract; idempotencyKey: string }
  | { type: "saveCostEntry"; cost: CostEntry; idempotencyKey: string }
  | { type: "savePayroll"; payroll: Payroll; idempotencyKey: string }
  | { type: "registerPayment"; financialEntryId: string; amountCents: number; paymentDate: string; paymentMethod: string; idempotencyKey: string }
  | { type: "saveMeeting"; meeting: Hearing; idempotencyKey: string }
  | { type: "cancelMeeting"; meetingId: string; reason: string; idempotencyKey: string }
  | { type: "createLeadFollowUp"; leadId: string; dueDate: string; idempotencyKey: string }
  | { type: "setDefaultLeadSource"; sourceId: string; idempotencyKey: string };
...
export type AppState = {
  ...
  processPhaseHistory: ProcessPhaseHistory[];
  deadlines: Deadline[];
  tasks: Task[];
...
export type FeaturePageProps = {
  ...
  executeAtomic: (operation: AtomicOperation) => Promise<void>;
};
