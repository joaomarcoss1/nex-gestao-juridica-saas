...
export type FieldKind = "text" | "number" | "select" | "date" | "textarea" | "email" | "password" | "checkbox" | "datetime-local";
...
export function EntityFormModal<T extends { id: string }>({
  ...
  function update(key: keyof T, raw: string | boolean, kind?: FieldKind) {
    setDraft((current) => ({ ...current, [key]: kind === "number" ? Number(raw) || 0 : kind === "checkbox" ? Boolean(raw) : raw }));
  }
  ...
  async function submit() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Não foi possível salvar este registro.";
      setErrors([message]);
    } finally {
      setSaving(false);
    }
    ...
    <div className="form-grid">
      {fields.map((field) => {
        ...
        if (field.kind === "checkbox") {
          return <label key={String(field.key)} className="checkbox-field"><input type="checkbox" checked={Boolean(rawValue)} disabled={field.readOnly} onChange={(e) => update(field.key, e.target.checked, field.kind)} /><span>{field.label}</span></label>;
        }
