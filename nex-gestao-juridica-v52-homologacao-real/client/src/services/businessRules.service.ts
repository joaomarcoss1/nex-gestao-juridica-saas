...
export function deriveOperationalStatuses(state: AppState): AppState {
  ...
  return {
    ...state,
    finances: state.finances.map((entry) => ["Pendente", "Parcial"].includes(entry.status) && !entry.archivedAt && !entry.cancelledAt && entry.dueDate < today && entry.amount > (entry.paidAmount ?? 0) ? { ...entry, status: "Atrasado" } : entry),
    deadlines: state.deadlines.map((deadline) => deadline.status === "Pendente" && deadline.dueDate < today ? { ...deadline, status: "Atrasado" } : deadline),
    tasks: state.tasks.map((task) => ["Pendente", "Em andamento"].includes(task.status) && !task.archivedAt && task.due < today ? { ...task, status: "Atrasada" } : task),
  };
}
